...
##############################################################################
#
# Copyright (c) 2008 Syleam Info Services (http://www.syleam.fr)
#               All rights Reserved.
...
class sale_simulator(orm.Model):
    ...
    _columns = {
        ...
        'line_ids': fields.one2many('sale.simulator.line', 'simul_id',
                                    'Lines'),
        'user_id': fields.many2one('res.users', 'Salesman', required=True,
                                   help="Salesman user"),
        'shop_id': fields.many2one(
            'sale.shop', 'Shop', required=True,
            help='Select shop to convert this sale as sale order'),
        'company_id': fields.many2one('res.company', 'Company', required=True),
        'fiscal_position': fields.many2one(
            'account.fiscal.position', 'Fiscal Position'),
        'validity_date': fields.date(
            'Validity date', help='Date validity for this quotation'),
    }

    _defaults = {
        'name': lambda obj, cr, uid,
        context: obj.pool['ir.sequence'].get(cr, uid, 'sale.simulator'),
        'company_id': lambda self, cr, uid,
        c: self.pool['res.company']._company_default_get(
            cr, uid, 'sale.simulator', context=c),
        'shop_id': _get_default_shop,
        'user_id': lambda obj, cr, uid, context: uid,
        'fiscal_position': False,
        'validity_date': lambda *a: datetime.strftime(
    ...
    def onchange_partner(self, cr, uid, ids, partner_id, context=None):
        ...
        if partner_id:
            ...
            if partner:
                res['pricelist_id'] = partner.property_product_pricelist and \
                    partner.property_product_pricelist.id or False
                res['fiscal_position'] = partner.property_account_position \
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        default['name'] = self.pool['ir.sequence'].get(
            cr, uid, 'sale.simulator')
        return super(sale_simulator, self).copy(
...
class sale_simulator_line(orm.Model):
    ...
    def _factory_price(self, cr, uid, ids, name, arg, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            mod_ids = line_obj.search(
                cr, uid, [('line_id', '=', line.id)], context=context)
            if mod_ids:
    ...
    _columns = {
        ...
        'simul_id': fields.many2one('sale.simulator', 'Sale simulator',
                                    select=True, required=True,
                                    ondelete='cascade'),
        'item_id': fields.many2one('product.item', 'Product Item',
                                   required=True,
                                   ondelete='cascade'),
        'quantity': fields.float('Quantity'),
        'factory_price': fields.function(
            _factory_price, digits_compute=dp.get_precision('Product Price'),
            method=True, type='float', string='Factory price'),
        'discount': fields.float('Discount'),
        'retail_price': fields.function(
            _retail_price, digits_compute=dp.get_precision('Product Price'),
            method=True, type='float', string='Retail price'),
        'margin': fields.function(_margin, method=True, type='float',
                                  string='Margin'),
        'sale_price': fields.float(
            'Sale price', digits_compute=dp.get_precision('Product Price')),
        'line_ids': fields.one2many(
            'sale.simulator.line.item', 'line_id', 'Item'),
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        return super(sale_simulator_line, self).copy(
            cr, uid, id, default, context=context)
    ...
    def compute_price(self, cr, uid, ids, context=None):
        ...
        if len(ids) > 1:
            raise orm.except_orm(
                _('Error'), _('You can only compute one line at the time!'))
        ...
        partner_id = line.simul_id.partner_id and \
            line.simul_id.partner_id.id or False
    ...
    def check_config(self, cr, uid, ids, context=None):
        ...
        for c in config:
            ...
            item_id = c['item_id'][0]
            f_id = f_obj.search(
                cr, uid, [('item_id', '=', item_id)], context=context)
            if not f_id:
                print '_check_config:f_id: not found !'

            feature_ids = f_obj.read(
                cr, uid, f_id, ['id', 'feature_id', 'quantity', 'global'],
                context=context)
            ...
            for z_id in c['line_ids']:
                module_id = sl_obj.read(
                    cr, uid, z_id, ['id', 'item_id2'], context=context)
                if not module_id:
                    raise orm.except_orm(
                        _('Error'), _('No module found for %d !') % z_id)

                fmod_args = [('item_id', '=', module_id['item_id2'][0])]
                fmod_ids = f_obj.search(cr, uid, fmod_args, context=context)
                if fmod_ids:
                    fitem_ids = f_obj.read(
                        cr, uid, fmod_ids, ['id', 'feature_id', 'quantity',
            ...
            for k in tf.keys():
                if k in lf and tf[k] < lf[k]:
                    ...
                    rew = self.write(cr, uid, ids, {
                        'message': 'Feature (%s) overload vals:%s max:%s'
                        % (nf[k], str(lf[k]), str(tf[k]))
                    }, context=context)

            if msg:
                rew = self.write(
                    cr, uid, ids, {'message': 'Configuration valid'},
                    context=context)
                if not rew:
                    raise orm.except_orm(
                        _('Error'),
                        _('Cannot print error message on the line'))
    ...
    def onchange_saleprice(
            self, cr, uid, ids, item_id, retail_price, sale_price):
        v = {}
        if item_id:
            if retail_price <= 0:
                v['discount'] = 0
    ...
    def _assembly_bom_from_line(self, cr, uid, line, context=None):
        ...
        for p in line.item_id.item_ids:
            if p.product_id.id in tmp_list.keys():
                tmp_list[p.product_id.id] = [
                    tmp_list[p.product_id.id][0] + p.quantity, p.uom_id.id]
            else:
        ...
        for opt in line.line_ids:
            for m in opt.item_id2.item_ids:
                if m.product_id.id in tmp_list.keys():
                    tmp_list[m.product_id.id] = [
                        tmp_list[m.product_id.id][0] + m.quantity, m.uom_id.id]
                else:
    ...
    def create_product_with_bom(self, cr, uid, id, procode='', context=None):
        ...
        product_ids = product_obj.search(cr, uid, [(
            'default_code', '=', procode)], offset=0, limit=None, order=None,
            context=context)
        if product_ids:
        ...
        bom = {
            ...
            'routing_id': line.item_id.routing_id and
            line.item_id.routing_id.id or False,
            'bom_lines': [],
        }
        if line.item_id.company_id:
        ...
        self.pool['mrp.bom'].create(cr, uid, bom, context=context)
        ...
        if line.item_id.supplier_id:
            ...
            product_obj.write(
                cr, uid, [product_id], {'seller_ids': [(0, 0, supplier_form)]},
                context=context)
    ...
    def create_sale_order(self, cr, uid, ids, context=None):
        ...
        if len(ids) > 1:
            raise orm.except_orm(
                _('Error'),
                _('You can only create one sale order at the time!'))

        line = self.browse(cr, uid, ids[0], context=context)
        if not line:
            raise orm.except_orm(
                _('Error'),
                _('Line not found, please reload your browser!'))

        if not line.simul_id.partner_id:
            raise orm.except_orm(
                _('Error'),
                _('A partner is necessary before create a sale order!'))

        # if line.simul_id.validity_date < fields.date.today:
        #     raise orm.except_orm(_('Error'),
        # _('Validity is outdated, you cannot create a sale order!'))
        ...
        product_id = self.create_product_with_bom(
            cr, uid, ids[0], procode=final_code, context=context)

        # Search for invoicing and delevrey addresses
        addr = self.pool['res.partner'].address_get(
            cr, uid, [line.simul_id.partner_id.id],
            ['delivery', 'invoice', 'contact'], context=context)

        # retrieve fiscal position to compute
        fiscal_position = line.simul_id.fiscal_position
        fpos = fiscal_position and self.pool.get(
            'account.fiscal.position').browse(cr, uid, fiscal_position.id) \
            or False
        ...
        sorder = {
            ...
            'order_line':
                [(0, 0, {
                    ...
                    'tax_id':
                    [(6, 0,
                        self.pool.get('account.fiscal.position').map_tax(
                            cr, uid, fpos, line.item_id.sale_taxes_id))],
                    'price_unit': line.sale_price,
                })],
        }

        order_id = self.pool['sale.order'].create(
            cr, uid, sorder, context=context)
    ...
    def cancel_sale_order(self, cr, uid, ids, context=None):
        ...
        if not line.order_id:
            raise orm.except_orm(
                _('Error'),
                _('Oups you may not be here!\nNo sale order to cancel'))

        self.pool['sale.order'].action_cancel(
            cr, uid, [line.order_id.id], context=context)

        return self.write(
            cr, uid, [line.id], {'order_id': False}, context=context)
...
class sale_simulator_line_item(orm.Model):
    ...
    def name_get(self, cr, uid, ids, context=None):
        ...
        res = []
        for read in self.read(cr, uid, ids, ['line_id', 'item_id2'],
                              context=context):
    ...
    _columns = {
        ...
        'line_id': fields.many2one(
            'sale.simulator.line', 'Sale simulator line',
            required=True, select=True),
        'item_id2': fields.many2one('product.item', 'Product Item',
    ...
    def onchange_item(self, cr, uid, ids, item_id, context=None):
        ...
        v = {}
        item = self.pool['product.item'].browse(
